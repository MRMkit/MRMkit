...
with open("quant_auc_rt.txt") as quant:
    ...
    for line in quant:
        ...
        if lsp[starti + blocklen]:
            norm_dat.append(
                lsp[:starti] + [float(lsp[starti + blocklen + fo]) for fo in file_ord]
            )
...
def gp_reg(ss, st0, en0):
    ...
    if corr0:
        Efs_yy = [
            sum(dnorm_dict[xs1 - xx2] * yy[xxj] for xxj, xx2 in enumerate(xx))
            / sum(dnorm_dict[xs1 - xx2] for xx2 in xx)
            for xs1 in xs
        ]
    else:
...
with PdfPages("run_seq_correction.pdf") as pdf0, PdfPages("run_seq_log2.pdf") as pdf1:
    ...
    for nn, nd0 in enumerate(unnorm_dat[:]):
        ...
        for cp0 in change_pt:
            plt.axvline(x=cp0 - 0.5, color="k", lw=1)
        nd_ = nd0[starti:]
        log2nd = np.log2(nd_)
        ...
        plt.scatter(seq0sam, log2nd_, s=2, color="k", alpha=0.2)

        plt.legend()
        q1, q3 = np.quantile([x for x in log2nd if np.isfinite(x)], [0.1, 0.9])
        ub = q3 + max(1, q3 - q1)
        lb = q1 - max(1, q3 - q1)
    ...
    for nn, nd0 in enumerate(norm_dat[:]):
        ...
        if plot0:
            ...
            for cp0 in change_pt:
                ax0.axvline(x=cp0 - 0.5, color="k", lw=1)
                ax1.axvline(x=cp0 - 0.5, color="k", lw=1)
        ...
        if plot0:
            ...
            log2nd_ = [x for x, y in zip(log2nd, type_l) if y == "SAMPLE"]
            ax0.scatter(seq0sam, log2nd_, s=2, color="k", alpha=0.2)
            ax0.legend()
            q1, q3 = np.quantile([x for x in log2nd if np.isfinite(x)], [0.1, 0.9])
            ub = q3 + max(1, q3 - q1)
            lb = q1 - max(1, q3 - q1)
            ax0.set_ylim(lb - 0.5, ub + 0.5)
        ...
        for adj0, cp0, cp1 in zip(adj0_l, [0] + change_pt, change_pt + [len(batch_l)]):
            ...
            adj0_ = [
                x
                for x, y in zip(adj0, type_l[cp0:cp1])
                if np.isfinite(x) and y != "TQC"
            ]
            ...
            nd1 = [
                (x - adjmed + allmed if y != "TQC" else tqcy)
                for y, x, tqcy in zip(type_l[cp0:cp1], adj0, np.log2(nd_[cp0:cp1]))
            ]

            adj_dat0.extend(nd1)
            if corr0 and plot0:
                ss = [
                    (y, z) for x, y, z in zip(type_l[cp0:cp1], seq1, nd1) if x != "TQC"
                ]
                xy = gp_reg(ss, cp0, cp1)
        ...
        if plot0:
            ...
            ax1.scatter(seq0sam, adj_dat0_, s=2, color="k", alpha=0.2)
            q1, q3 = np.quantile([x for x in log2nd if np.isfinite(x)], [0.1, 0.9])
            ub = q3 + max(1, q3 - q1)
            lb = q1 - max(1, q3 - q1)
            ax1.set_ylim(lb - 0.5, ub + 0.5)
...
with open("batch_adjusted.txt") as my_op:
    ...
    for line in my_op:
        ...
        if lsp[2] == "BQC":
            batchBQC.add(lsp[1])
            for l, cpdname, Q1, Q3, RT in zip(
                lsp[starti:], mylsp1, mylspQ1, mylspQ3, mylspRT
            ):
                my_BQCdict[lsp[1] + "\t" + cpdname + "\t" + Q1 + "\t" + Q3].append(
                    float(l)
                )
        elif lsp[2] == "TQC":
            batchTQC.add(lsp[1])
            for l, cpdname, Q1, Q3, RT in zip(
                ...
                my_TQCdict[lsp[1] + "\t" + cpdname + "\t" + Q1 + "\t" + Q3].append(
                    float(l)
                )
        elif lsp[2] == "SAMPLE":
            batchSAM.add(lsp[1])
            for l, cpdname, Q1, Q3, RT in zip(
                ...
                my_SAMdict[lsp[1] + "\t" + cpdname + "\t" + Q1 + "\t" + Q3].append(
                    float(l)
                )
    for k, v in my_SAMdict.items():
        q1, q3 = np.quantile(v, [0.1, 0.9])
        ub = q3 + max(1, q3 - q1)
        lb = q1 - max(1, q3 - q1)
...
with open("BQC_CoV.txt", "w") as qctab:
    qctab.write(
        "name\tQ1\tQ3\tRT\tave #peaks\t%CoV_all\t"
        + "\t".join("%CoV_" + x for x in batchBQC)
        + "\n"
    )
    for cpdname, Q1, Q3, RT in zip(mylsp1, mylspQ1, mylspQ3, mylspRT):
        qctab.write("\t".join((cpdname, Q1, Q3, RT)))
        qctab.write(
            "\t{}".format(len(npeaks[(cpdname.rsplit(" RT", 1)[0], Q1, Q3, RT)]))
        )
        selquantBQC = [
            xx
            for b0 in batchBQC
            for xx in my_BQCdict[b0 + "\t" + cpdname + "\t" + Q1 + "\t" + Q3]
        ]
        ...
        for b0 in batchBQC:
            ...
            qctab.write(
                "\t{:.1f}".format(100 * np.std(selquantBQC) / np.mean(selquantBQC))
            )
        qctab.write("\n")
...
with open("TQC_CoV.txt", "w") as qctab:
    ...
    for cpdname, Q1, Q3, RT in zip(mylsp1, mylspQ1, mylspQ3, mylspRT):
        ...
        selquantTQC = [
            xx
            for b0 in batchTQC
            for xx in my_TQCdict[b0 + "\t" + cpdname + "\t" + Q1 + "\t" + Q3]
        ]
        ...
        for b0 in batchTQC:
            ...
            qctab.write(
                "\t{:.1f}".format(100 * np.std(selquantTQC) / np.mean(selquantTQC))
            )
        qctab.write("\n")
...
with open("D_ratio.txt", "w") as qctab:
    qctab.write(
        "name\tQ1\tQ3\tRT\tave #peaks\t%D-ratio_all\t"
        + "\t".join("%D-ratio_" + x for x in batchTQC)
        + "\n"
    )
    for cpdname, Q1, Q3, RT in zip(mylsp1, mylspQ1, mylspQ3, mylspRT):
        qctab.write("\t".join((cpdname, Q1, Q3, RT)))
        ...
        selquantSAM = [
            xx
            for b0 in batchSAM
            for xx in my_SAMdict[b0 + "\t" + cpdname + "\t" + Q1 + "\t" + Q3]
        ]
        selquantBQC = [
            xx
            for b0 in batchBQC
        ...
        for b0 in batchSAM:
            ...
            selquantBQC = my_BQCdict[b0 + "\t" + cpdname + "\t" + Q1 + "\t" + Q3]
            qctab.write(
                "\t{:.1f}".format(100 * np.std(selquantBQC) / np.std(selquantSAM))
            )
        qctab.write("\n")
...
for (cpd, q1, q3, rt), rt_l in npeaks.items():
    if len(rt_l) == 1:
        class_rt[t_list[bisect_left(t_name, cpd)].lclass].append(
            (float(rt), float(rt_l[0]))
        )
...
with PdfPages("expected_detected.pdf") as pdf0:
    ...
    for (size0, color0), (class0, rt0) in zip(
        itertools.product(range(9), "bgrcmyk"), class_rt.items()
    ):
        xx = [x for x, _ in rt0]
        yy = [x for _, x in rt0]
        ax0.plot(
...
with open(ppfile, "w") as usersRT:
    usersRT.write(
        "class\tname\tQ1\tQ3\t%CoV(TQC)\t%CoV(BQC)\tD-ratio\texpectedRT\tdetectedRT\tusersRT\n"
    )
    for (cpd, q1, q3, rt), npeak in sorted(npeaks.items()):
        with open(
            os.path.join(
                miscdir,
                commonfn.format_filename("trans2_{}Q1_{}Q3_{}.txt".format(cpd, q1, q3)),
            )
        ) as trans2:
            next(trans2)
        ...
        usersRT.write(
            "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                ...
                q1,
                q3,
                ", ".join(CoVTQC[(cpd, q1, q3)]),
                ", ".join(CoVBQC[(cpd, q1, q3)]),
                ", ".join(Dratio[(cpd, q1, q3)]),
                rt,
                ", ".join(npeak),
                pred,
            )
