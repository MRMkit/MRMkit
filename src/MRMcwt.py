...
def findridge0(wave_scales, wave_sqrt, rerun, rerunw, EIC):
        ...
        for yy, wave_loc in enumerate(eic_rt):
            ...
            for i, rt0 in enumerate(rt_):
                if eic_dict[rt0] > 0:  # and rt0!=wave_loc:
                    ...
                    int_I[i] = eic_dict[rt0] * math.exp(-tsig2 / 2) * (1.0 - tsig2)
            coefs[xx][yy] = (
                sum(
                    (I0 + I1) * (rt1 - rt0)
                    for rt0, rt1, I0, I1 in zip(rt_, rt_[1:], int_I, int_I[1:])
                )
                / 2
                / wave_sqrt[xx]
            )
    ...
    for xx, wave_scale in enumerate(wave_scales):
        ...
        while any(y > 0 for _, y in coef_xx):
            ...
            if (
                max_i == 0
                or max_i == len(coef_xx) - 1
                or coef_xx[max_i - 1][1] == 0
                or coef_xx[max_i + 1][1] == 0
            ):
        ...
        for yy, coef in enumerate(scale_coef):
            ...
            if coef:
                for rl in ridgelines:
                    if (
                        abs(
                            bisect_left(rt_all, rl[-1].rt)
                            - bisect_left(rt_all, eic_rt[yy])
                        )
                        < 2
                        or abs(rl[-1].rt - eic_rt[yy]) < 1
                    ) and rl[-1].sc == wave_scales[xx - 1]:
    ...
    if not rerun:
        for rd in ridgelines[:]:
            ...
            if pos0 == 0 or pos1 == len(
                rt_all
            ):  # or sum(1 for rt0 in rt_all[pos0:pos1] if rt0 in eic_dict)/(pos1-pos0)<.5:
                ridgelines.remove(rd)
                continue
    ...
    if not rerun:
        for rd in ridgelines:
            ...
            if (
                pos2 < pos0 - 1
                and max(eic_dict[rt0] for rt0 in rt_all[pos2 : pos0 - 1]) < apex_h
            ):
                pos0 = pos2
            if (
                pos1 + 1 < pos3
                and max(eic_dict[rt0] for rt0 in rt_all[pos1 + 1 : pos3]) < apex_h
            ):
                pos1 = pos3
            auc = (
                sum(
                    (eic_dict[rt0] + eic_dict[rt1]) * (rt1 - rt0)
                    for rt0, rt1 in zip(rt_all[pos0:], rt_all[pos0 + 1 : pos1])
                )
                / 2
            )
            if 0 < auc:
        ...
        for rd in ridgelines:
            ...
            peaks.append(
                Peak(peak_loc.rt, peak_loc.sc, len(rd), peak_loc.coef, auc, None, None)
            )
    ...
    for nn, peak1 in enumerate(peaks_):
        for p in peaks_[nn + 1 :]:
            if p in peaks and abs(peak1.rt - p.rt) < 0.1:
                peaks.remove(p)
    if not peaks:
        sc = 10
