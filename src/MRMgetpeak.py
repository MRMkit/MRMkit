...
miscdir='trans'
trans2filename={trans:commonfn.format_filename('{}Q1_{:.1f}Q3_{:.1f}.txt'.format(trans.name,trans.q1,trans.q3)) for trans in t_list}#+t_list2}
trans1filename={trans:os.path.join(miscdir,'trans_'+t)for trans,t in trans2filename.items()}
trans2filename={trans:os.path.join(miscdir,'trans2_'+t)for trans,t in trans2filename.items()}

t_list=[x for x in t_list if x.istd in istd_dict and os.path.isfile(trans1filename[x])]
...
def write_to_trans(trans):
    ...
    with open(trans1filename[trans]) as feat_in, open(trans2filename[trans],'w') as fo:
        ...
        fo.write('{}\n'.format(trans.name))
        for ii,(eicfeat,istd_eicfeat) in enumerate(zip(get_EIC_feats(feat_in),get_EIC_feats(istd_in))):
            rt_,I_,*_=eicfeat
            firstdecile=sorted(I_)[int(len(I_)/10)]
            I_=[max(0,i-firstdecile) for i in I_]
            ...
            if ISTD_trace_all:
                for feat_rt,cc in new_rt:
                    ...
                    fo.write('\t')
                    fo.write('\t'.join(str(x/apex_istd*apex_int+firstdecile) for _,x in RT_I))
                    fo.write('\n')
                fo.write('\n')
            else:
...
if __name__ == '__main__':
    ...
    def qq_rt(secondr):
        ...
        for trans in t_list:
            trans_file=os.path.join(miscdir,commonfn.format_filename(wstr0+'{}Q1_{:.1f}Q3_{:.1f}.txt'.format(trans.name,trans.q1,trans.q3)))
    ...
    with open('quant_auc_rt.txt','w') as quant_:
        quant_.write('name\tprecursor\tproduct\tRT\t{}\t{}\t{}\n'.format('\t'.join(mzML_files),'\t'.join('norm_'+x for x in mzML_files),'\t'.join('npeaks_'+x for x in mzML_files)))
        for trans,auc_rt_l in QQ_rt2.items():

            istd0=istd_dict[trans.istd]
            for jj in range(len(auc_rt_l[0])):
                medianRT=statistics.median(auc_rt_l[ii][jj][0]for ii in range(len(mzML_files)))
                quant_.write('{} RT{:.0f}\t{:.1f}\t{:.1f}\t{}'.format(trans.name,medianRT,trans.q1,trans.q3,trans.rt))
                    ...
                    if trans.name in istd_dict:
                        ...
                        for ii in range(len(mzML_files)):
                            ...
                            if auc_istd_==0:
                                ...
                                quant_.write('\t{}'.format(auc_rt_l[ii][jj][1]/auc_istd_))
                else:
                    for ii in range(len(mzML_files)):
                        quant_.write('\t{}'.format(auc_rt_l[ii][jj][1]))
