...
trans2filename = {
    trans: os.path.join(
        ...
        commonfn.format_filename(
            "trans_{}Q1_{:.1f}Q3_{:.1f}.txt".format(trans.name, trans.q1, trans.q3)
        ),
    )
    for trans in t_list
}
...
def write_to_trans(eic_dict, trans):
    ...
    if trans.name == trans.istd:
        ...
        if len(feats_) > 1:
            ...
            feats = [
                commonfn.Peak(
                    feats_[0].rt, feats_[0].sc * 2, None, None, auc, begin0, end0
                )
            ]
    else:
        feats = []
...
for sta_ in range(0, len(mzML_files), nfile):
    ...
    def write_block(trans):
        with open(trans2filename[trans], "a") as fo, open(
            "missing_compounds.txt", "a"
        ) as miss_cpd:
            for mzml, eic_dict in zip(mzML_files[sta_ : sta_ + nfile], eic_dict_all):
                ...
                for feat in feats:
                    fo.write(
                        "{:.3f}\t{}\t{:.3f}\t{:.3f}\t{:.3f}\n".format(
                            feat.rt, feat.auc, feat.begin, feat.end, feat.sc
                        )
                    )
                fo.write("\n")
...
for trans_file in glob.glob(os.path.join(miscdir, "trans_*Q1_*Q3_*.txt")):
    ...
    if cs < len(mzML_files):
        print(
            "removed {}. Present in {}/{} samples".format(
                trans_name, cs, len(mzML_files)
            )
        )
        miss_cpd.write(
